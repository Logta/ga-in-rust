use crate::models::model::{BaseModel, Model, Agent};
...
pub fn get_new_game<T, U>(ga: GA<T>) -> Game<T, U>
where
    T: BaseModel,
    U: StrategyOperation<T>
{
    let agents = (0..ga.population).map(|x| model::new_base_model(x,get_dna(ga.old_agents, ga.population, ga.mutation_rate))).collect();

    game::generate_next_game(ga.population, ga.mutation_rate, ga.num_game,ga.dna_length, agents)
}

fn get_dna(agents: &Vec<Box<Agent>>, poplation: u64, mutation_rate: f64) -> String{
...
fn choose_model_parent(agents: &Vec<Box<Agent>>, poplation: u64) -> (Agent, Agent){
...
fn choose_model_roulettes(agents: &Vec<Box<Agent>>, poplation: u64, sum_point: u64) -> Agent {
